...
const EPUB_INPUT = process.env.EPUB_FILE ?? DEFAULT_EPUB_FILE
const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url))

// 从文件名提取书名（去掉扩展名）
const BOOK_NAME = parse(EPUB_INPUT).name
...
function resolveEpubPath(inputPath) {
  const candidates = [
    resolve(process.cwd(), inputPath),
    resolve(SCRIPT_DIR, inputPath),
  ]
  ...
  throw new Error(
    `Invalid/missing file ${inputPath}. Checked: ${candidates.join(", ")}`
  )
}
...
async function loadAndProcessEPubStreaming(bookId) {
  try {
    const epubFile = resolveEpubPath(EPUB_INPUT)
    console.log(`\n开始加载 EPUB 文件: ${epubFile}`)
