...
// 顶层使用对象包裹数组，避免兼容 OpenAI 的部分实现对顶层数组支持不稳定
const friendsExtractionSchema = z.object({
  friends: z.array(friendSchema).describe("好友信息数组"),
})

// 使用 withStructuredOutput 方法
const structuredModel = model.withStructuredOutput(friendsExtractionSchema, {
  method: "jsonSchema",
  name: "extract_friends",
  strict: true,
})
    ...
    const messages = [
      [
        ...
        [
          ...
          "The top-level JSON must be an object with a single key: friends.",
          "The friends field must be a JSON array.",
          "Use null for missing fields.",
        ].join(" "),
      ],
      [
        "human",
        `请从以下文本中提取所有好友信息，文本中可能包含一个或多个人的信息。请将每个人的信息分别提取出来，并放到 friends 数组中返回。
...
4. 返回格式必须是 JSON 对象，且顶层只有一个字段 friends
5. friends 必须是 JSON 数组，即使只有一个人也要放在数组中`,
      ],
    ]

    const result = await structuredModel.invoke(messages)
    const results = result.friends
...
async function main() {
  ...
  try {
    ...
    console.error("❌ 处理失败：", error.stack || error)
    process.exit(1)
  }
}
